...
import org.esupportail.commons.services.smtp.MessageIdChangingMimeMessage;
...
/**
 * A class that provides utilities to send emails.Partly Duplicates the SmtpUtils of Esup-commons but add an acknowldegment to the mails.
 */
public class SmtpUtils {
	...
	/**
	 * Send an email with acknowledgment
	 * @param smtpServers 
	 ...
	 */
	public static void sendEmailWithAck(
		...
		SmtpUtils.sendEmailWithAck(smtpServers, from, to, subject, htmlBody, textBody, files, charset, null, recipientMessage, acknowledgementMessage);
	}
	...
	public static void sendEmailWithAck(
		...
		try {
			for (SmtpServer smtpServer : smtpServers) {
				...
				//Add an acknowledgment to the mail except if it's an exception/error to the specified address acknowledgementMessage
				if (!to.toString().equalsIgnoreCase(recipientMessage) && !to.toString().contains(recipientMessage))
					message.addHeader("Disposition-Notification-To", acknowledgementMessage);
				// Send message
	...
	public static void sendEmailtocc(
		...
		SmtpUtils.sendEmailtocc(
				...
				subject, htmlBody, textBody, files, charset, null, recipientMessage, acknowledgementMessage);
	}
		...
		try {
			for (SmtpServer smtpServer : smtpServers) {
				...
				message.addHeader("Subject", encodedSubject);
				//Add an acknowledgment to the mail except if it's an exception/error to the specified address acknowledgementMessage
				if (!tos.toString().equalsIgnoreCase(recipientMessage) && !tos.toString().contains(recipientMessage))
					message.addHeader("Disposition-Notification-To", acknowledgementMessage);
